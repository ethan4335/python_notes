...
generate_recover = False # 生成还原文件，还是读取文件
input_file = 'new_layout'
        ...
        with open(input_file, encoding="utf-8") as f:
            ...
            try:
                ...
                if save_format == "bat":
                    ...
                    with open("layout_bat/" + '.'.join(input_file.split('\\')[-1].split('.')[:-1]) + '.bat', 'w', encoding="utf-8") as g:
                    ...
                    input("...\n{}文件已生成至layout_bat目录下，右键以管理员身份执行该文件后重启系统生效。\n按回车键退出程序...".format('.'.join(input_file.split('\\')[-1].split('.')[:-1]) + '.' + save_format))

                else:
                    # with open("layout_reg/" + '.'.join(sys.argv[1].split('\\')[-1].split('.')[:-1]) + '.reg', 'w', encoding="utf-8") as g:
                    with open("layout_reg/" + '.'.join(input_file.split('\\')[-1].split('.')[:-1]) + '.reg', 'w', encoding="utf-8") as g:
                        g.write(
